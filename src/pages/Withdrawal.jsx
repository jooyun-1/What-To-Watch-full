...
const Withdrawal = () => {
  ...
  useEffect(() => {
    ...
    const fetchSessionID = async () => {
      try {
        const response = await axios.get('http://localhost:3100/api/session');
        setSessionID(response.data.sessionID);
      } catch (error) {
        console.error('세션 식별자 요청 실패:', error);
      }
    };
  ...
  const handleWithdrawal = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:3100/api/withdrawal', { password }, { headers: { Authorization: `Bearer ${sessionID}` } });
